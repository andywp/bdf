...
class MediaAdvisoryController extends Controller
{
    public function index(){
        return view('admin.mediai-advisory.index');
    }

    public function dataTable(Request $request){
        if ($request->ajax()) {
            $datas = Category::select('id','title','publish','created_at')->where('type','media-advisory');
            return DataTables::of($datas)
                        ->addColumn('action', function($row){  
                            //$enc_id = \Crypt::encrypt($row->id);
                            $btn = '
                                    <form id="fd'.$row->id.'" action="'.route("admin.advisory.destroy",$row->id).'" method="POST">
                        ...
                        ->addColumn('file', function($row){  
                            //$enc_id = \Crypt::encrypt($row->id);
                            $btn = '
                                    <a href="'.route("admin.advisory.list",$row->id).'" class="btn btn-outline-info btn-xs">Manage File</a>
                                    ';
                            return $btn;
                        })
    ...
    public function store(Request $request,Category $Category){
        ...
        $Category::create(['title' => $request->title ,'type' => 'media-advisory']);
        return response()->json(['success'=>'Media Advisory successfully Saved']);
    }
    ...
    public function update(Request $request,Category $Category){
        ...
        $data->title = $name;
        $data->save();
        return response()->json(['success'=>'Media Advisory successfully Updated']);
    }
    public function destroy(Request $request, $id, Category $Category){
        $id =(int) $id;
        $Category::find($id)->delete();
        return redirect()->route('admin.mediai-advisory.index')
    ...
    public function listPublication($id, Category $Category){
        ...
        return view('admin.mediai-advisory.list', compact('dataAlbum'));
    }
        ...
        if($request->ajax()){
            ...
            return DataTables::of($datas)
                        ->addColumn('action', function($row){  
                            //$enc_id = \Crypt::encrypt($row->id);
                            $btn = '
                                    <form id="fd'.$row->id.'" action="'.route("admin.advisory.destroyFile",$row->id).'" method="POST">
                                        <input type="hidden" name="_token" value="' . csrf_token() . '" />
                                        <input type="hidden" name="_method" value="DELETE">
                                        <div class="d-flex">
