...
class PostController extends Controller
{
    ...
    public function store(Request $request,Post $Post){
       ...
       $request->validate([
        ...
        if(!empty($file)){
            /* $post_image=Carbon::now()->format('YmdHis').Str::random(5).'.'.$file->extension();
            $ogImage = Image::make($file);
            $originalPath = $this->imagesPath;
            $ogImage =  $ogImage->save($originalPath.$post_image);

            $thumbnailPath = $this->imagesPath.'thumb/';
            ...
            $post_image = $ogImage->save($thumbnailPath.$post_image); */
    ...
    public function dataTable(Request $request){
        if ($request->ajax()) {
            $datas = Post::select('id','post_title','post_image','posts_author','published_date','publish','created_at');
            return DataTables::of($datas)
                        ->addColumn('action', function($row){  
                            //$enc_id = \Crypt::encrypt($row->id);
                            $btn = '
                                    <form id="fd'.$row->id.'" action="'.route("admin.article.destroy",$row->id).'" method="POST">
                                        <input type="hidden" name="_token" value="' . csrf_token() . '" />
                                        <input type="hidden" name="_method" value="DELETE">
                                        <div class="d-flex">
                                            <a href="'.route("admin.article.edit",$row->id).'" data-id="'.$row->id.'" data-toggle="tooltip"  data-original-title="Edit" class="edit btn btn-primary shadow btn-xs sharp me-1" ><i class="fas fa-pencil-alt"></i></a>
